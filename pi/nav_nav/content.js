(function () {
  ...
  function init() {
    console.log("nav_nav: Initializing content script...");
    if (document.getElementById("nav_nav-container")) return;
  ...
  function loadSettingsAndInit() {
    chrome.storage.local.get(["position", "urls"], async (result) => {
      console.log("nav_nav: Loaded local storage", result);
      ...
      try {
        ...
        if (response.ok) {
          ...
          console.log("nav_nav: Loaded config.json", config);
          if (config.position && !result.position)
        ...
        console.log("nav_nav: No config.json found or error loading it.", e);
        if (result.urls) settings.urls = result.urls;
      }
  ...
  function createUI() {
    ...
    navContainer.id = "nav_nav-container";

    navButton = document.createElement("button");
    navButton.id = "nav_nav-button";
    navButton.innerHTML = `
      <svg viewBox="0 0 24 24"><path d="M4 6h16v2H4zm0 5h16v2H4zm0 5h16v2H4z"/></svg>
    `;

    navMenu = document.createElement("div");
    navMenu.id = "nav_nav-menu";
    ...
    document.addEventListener("click", (e) => {
      ...
      const isInside =
        ...
        e.target.closest?.("#site-nav-container");
  ...
  function renderShortcuts() {
    ...
    if (settings.urls.length === 0) {
      list.innerHTML =
        '<div class="sn-empty-msg">No shortcuts saved. Click gear to add.</div>';
      return;
    }
